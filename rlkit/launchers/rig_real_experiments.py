        ...
        if use_cached and osp.isfile(filename):
            ...
            if not isinstance(env, ImageEnv):
                env = ImageEnv(
                    ...
                    recompute_reward=False,  # TUNG: avoid re-compute reward
                )
            else:
                imsize = env.imsize
...
def grill_her_td3_experiment(variant):
    ...
    env.wrapped_env.recompute_reward = False    # TUNG: disable compute reward
...
def get_video_save_func(rollout_function, env, policy, variant):
    ...
    if do_state_exp:
        ...
        def save_video(algo, epoch):
            if epoch != 0 and (epoch % save_period == 0 or epoch == algo.num_epochs):
                filename = osp.join(logdir,
                                    'video_{epoch}_env.mp4'.format(epoch=epoch))
                temporary_mode(
                    image_env,
