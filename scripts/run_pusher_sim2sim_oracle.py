...
from rlkit.envs.vae_wrapper_hack_sim2sim import VAEWrappedEnv
...
def wrap_real_env_manually(data_ckpt=None, env_name='SawyerPushNIPSCustomEasy-v0',
                           image_env_sim=None):
    """
...
def simulate_policy(args):
    ...
    image_env_sim = copy.deepcopy(env.wrapped_env)

    # Re-initialize REAL env wrapped by learned VAE (from SIM)
    env_manual = wrap_real_env_manually(data_ckpt=data, env_name='SawyerPushNIPSCustomEasy-v0',
                                        image_env_sim=image_env_sim)
    if args.gpu:
        ptu.set_gpu_mode(True)
    ...
    env_manual.wrapped_env.wrapped_env.randomize_goals = False  # Setup Mujoco env
